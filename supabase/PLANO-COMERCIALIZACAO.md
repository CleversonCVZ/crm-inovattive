> **Status (2026-09-15):** Fase A **completa para o primeiro beta tester** — Solv Automação (Fantasia: "Solv Automação"; Razão: "J&J Automação e Tecnologia"; CNPJ 60.837.250/0001-02). Ambiente 100% duplicado e isolado no ar:
> - Supabase próprio: projeto `bkyujsfggeskmohmtilw` ("Solv Automacao CRM"), schema completo (52 tabelas, FKs, índices, 127 RLS policies, 14 functions SECURITY DEFINER, triggers, 1 view) replicado do banco da Inovattive via introspecção direta (não por replay de migrations — ver nota técnica abaixo).
> - Storage: 13 buckets + policies recriados.
> - Edge Function `admin-usuarios` publicada.
> - Dados de configuração (grupos, fases, origens, funções, categorias) semeados com o mesmo template padrão da Inovattive.
> - Usuário administrador criado: `juanpablo@solvautomacao.com.br` (grupo Administradores).
> - Deploy Vercel próprio: projeto `crm-solv` (git-linked ao repo `CleversonCVZ/crm-solv`, criado à parte — não o mesmo repo do `crm-inovattive`), com `config.js` apontando pro banco da Solv. URL de produção: **https://crm-solv.vercel.app**.
> - Empresa cadastrada em Configurações do Sistema.
> - Validado ponta a ponta: login, logout, criação real de card+cliente, tudo funcionando.
>
> **Nota técnica importante (bug real encontrado e corrigido durante a validação):** a réplica inicial do schema, feita por introspecção via `information_schema`/`pg_catalog`, não capturou a propriedade `GENERATED BY DEFAULT AS IDENTITY` de 22 colunas `id` (ela não aparece em `column_default`, é um metadado separado do Postgres — `is_identity`/`identity_generation`). Isso quebrava a criação de QUALQUER registro nessas 22 tabelas (`cards`, `clientes`, `obras`, `propostas`, `audit_logs` etc.) enquanto elas estivessem vazias, porque o código nesses casos não informa o `id` no insert e espera que o Postgres gere sozinho. **Corrigido** via `ALTER TABLE ... ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY` nas 22 colunas do projeto da Solv. Isso é relevante pra **qualquer próximo beta tester**: ao replicar o schema de novo, checar `is_identity`/`identity_generation` em `information_schema.columns`, não só `column_default` — senão o ambiente novo quebra silenciosamente na primeira tentativa de criar qualquer registro em tabela vazia.
>
> Ainda no Vercel Hobby (free) por decisão do Cleverson — ver seção sobre ToS mais abaixo. Falta apenas: repetir os passos acima pra um 2º beta tester (se/quando houver), e, só depois de validar demanda real, decidir se avança pra Fase B (mono-instância).
>
> (Histórico: Passo 1 — extrair `SUPABASE_URL`/`SUPABASE_KEY` pro `config.js` — foi feito em 2026-09-15, v1.47.221/mobile v1.0.40, antes do resto da Fase A acima.)
